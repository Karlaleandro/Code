...
%calcula solução da equação do calor a u_xx = u_t for 0 <= x <= xf, 0 <= t <= T
% condição inicial: u(x,0) = it0(x)
% condição de fronteira: u(0,t) = bx0(t), u(xf,t) = bxf(t)
% M = # subintervalos ao longo do eixo x
% N = # subintervalos ao longo do eixo y
dx = xf/M; x = [0:M]'*dx;
dt = T/N; t = [0:N]*dt;
