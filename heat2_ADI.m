...
%resolve equação do calor u_t = c(u_xx + u_yy) for D(1) <= x <= D(2), D(3) <= y <= D(4), 0 <= t <= T
% Condição inicial: u(x,y,0) = ixy0(x,y)
% Condição de fronteira: u(x,y,t) = bxyt(x,y,t) 
% Mx = # de subintervalos para o eixo x
% My = # de subintervalos para o eixo y
% N = # de subintervalos para o tempo t
